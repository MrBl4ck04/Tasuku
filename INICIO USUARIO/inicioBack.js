document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelector('#categories ul').addEventListener('contextmenu', event => {
        event.preventDefault();
        const li = event.target.closest('li');
        if (li) {
            const categoryName = li.getAttribute('data-category');
            const confirmDelete = confirm(`¿Eliminar la categoría '${categoryName}' y todas sus tareas?`);
            if (confirmDelete) {
                // Eliminar categoría
                const categories = JSON.parse(localStorage.getItem('categories'));
                ...
                // Eliminar tareas asociadas a esta categoría
                const tasks = JSON.parse(localStorage.getItem('tasks'));
